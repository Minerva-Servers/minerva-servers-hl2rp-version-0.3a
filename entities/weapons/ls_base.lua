...
function SWEP:PrimaryAttack()
	...
	elseif clip >= 1 then
		...
		if ( self.PostPrimaryAttack ) then
			self:PostPrimaryAttack(self.Owner)
		end
	else
...
function SWEP:GetViewModelPosition( pos, ang )
	...
	if predicted then
		cacheAng = LerpAngle(math.Clamp(ft * 2.5, 0, 1), cacheAng, aDelta)
	end
	...
	if predicted then
		...
		if rd > 0.5 then
			c_look = Lerp(math.Clamp(ft * 5, 0, 1), c_look, 2 * c_move2)
		elseif rd < -0.5 then
	...
	ang.p = ang.p + (c_jump or 0) * 5
	ang.r = ang.r + c_look

	if bob != 0 and c_move > 0 then
		local p = c_move * c_sight * bob

		pos = pos - ang:Forward() * c_move * c_sight * fd - ang:Up() * c_move + ang:Right() * -1.5 * c_move * c_sight
...
function SWEP:DrawHUD()
	...
	if ( self.Attachments[attachment].ScopeTexture and self.Attachments[attachment].ScopeTexture != "" ) then
