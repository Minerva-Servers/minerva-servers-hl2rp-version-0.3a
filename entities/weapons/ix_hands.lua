...
function SWEP:PickupObject(entity)
	...
	self.holdEntity:SetCollisionGroup(COLLISION_GROUP_WEAPON)
...
function SWEP:SecondaryAttack()
	...
	if (SERVER and IsValid(entity)) then
		...
		elseif (entity:IsPlayer() and ix.config.Get("allowPush", true)) then
			local direction = self:GetOwner():GetAimVector() * (math.random(100, 300) + (self:GetOwner():GetCharacter():GetAttribute("str", 0) * 3))
				direction.z = 0
