...
ITEM:Hook("drop", function(item)
    ...
    if (item:GetData("equip")) then
        ...
        if (IsValid(weapon)) then
            ...
            owner:EmitSound(item.useSound or "minerva/global/unequip.mp3", 45)
        end
...
function ITEM:Equip(client, bNoSelect, bNoSound)
    ...
    if (IsValid(weapon)) then
        ...
        if (!bNoSound) then
            client:EmitSound(self.useSound or "minerva/global/equip.mp3", 45)
        end
...
function ITEM:Unequip(client, bPlaySound, bRemoveItem)
    ...
    if (bPlaySound) then
        client:EmitSound(self.useSound or "minerva/global/unequip.mp3", 45)
    end
