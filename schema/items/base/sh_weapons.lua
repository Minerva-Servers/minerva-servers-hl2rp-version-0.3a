...
ITEM:Hook("drop", function(item)
    ...
    if (item:GetData("equip")) then
        ...
        if (IsValid(weapon)) then
            ...
            owner:EmitSound(item.useSound or "minerva/global/unequip.mp3", 80)
        end
    end
...
function ITEM:WearPAC(client)
    if (ix.pac and self.pacData) then
        if not ( string.find(client:GetModel(), "/hl2rp/") ) then
            return
        end
...
function ITEM:Equip(client, bNoSelect, bNoSound)
    ...
    if (IsValid(weapon)) then
        ...
        if (!bNoSound) then
            client:EmitSound(self.useSound or "minerva/global/equip.mp3", 80)
        end
        ...
        weapon.ixItem = self
		weapon.durability = self:GetData( "durability", 100 )
...
function ITEM:Unequip(client, bPlaySound, bRemoveItem)
    ...
    if (IsValid(weapon)) then
		...
		self:SetData( "durability", weapon.durability )
        client:StripWeapon(self.class)
    else
    ...
    if (bPlaySound) then
        client:EmitSound(self.useSound or "minerva/global/unequip.mp3", 80)
    end
...
hook.Add("PlayerInteractItem", "ixPac3PlayerInteractItem", function(client, action, item)
...
if ( SERVER ) then
	...
	/* hook.Add( "EntityFireBullets", "IXWeaponDurability", function( cl, data )
		if ( !cl:IsPlayer() ) then
			return
		end

		data.Callback = function( en, tr, info )
			...
			wep.shotcount = wep.shotcount or 0
			wep.shotcount = wep.shotcount + 1

			if ( wep.shotcount > math.random( 5, 20 ) ) then
				wep.durability = wep.durability or 100
			...
			if ( wep.durability <= 0 ) then
				...
				local resources = item_to_resources[ wep:GetClass() ]
				...
				local character = cl:GetCharacter()
