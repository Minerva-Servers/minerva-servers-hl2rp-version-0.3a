...
ix.chat.Register("ptradio", {
    ...
    CanSay = function(self, speaker, text)
        return speaker:IsCombine() and speaker.curTeam
    end,
    OnChatAdd = function(self, speaker, text)
        ...
        chat.AddText(Color(21, 56, 146), "[PT-RADIO] "..speaker:Nick().." <:: ", text, " ::>")
    end,
    CanHear = function(self, speaker, receiver)
        return speaker.curTeam == receiver.curTeam
    end,
})
...
ix.command.Add("PTKick", {
	...
	OnRun = function(self, client, target)
		...
		if (client.curTeam != target.curTeam and !client:IsCPCommand()) then return "@TargetNotSameTeam" end

		if (!client.isTeamOwner and !client:IsCPCommand()) then return "@CannotKickTeamMembers" end
