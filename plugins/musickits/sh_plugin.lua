...
ix.lang.AddTable("english", {
    ...
    optdMusicKit = "What Music Kit do you want?",
...
local function ReadMusicKit(name)
    ...
    print("[ix] [musickits] Loaded "..json.Name.." ("..name..") music kit.")
end
...
local function GetMusicKits()
    ...
    for v,k in pairs(kits) do
        ...
        if err then
            print("[ix] [musickits] Failed to load "..k.." | "..err)
            continue
        end
    end
end
...
ix.option.Add("musicAmbientVol", ix.type.number, 1, {
    category = "Music Kits",
    min = 0,
    max = 100,
})
ix.option.Add("musicCombatVol", ix.type.number, 1, {
    category = "Music Kits",
    min = 0,
    max = 100,
})
...
local function InCombat() -- simple for now
    ...
    if ( LocalPlayer().inCombat and LocalPlayer().inCombat > CurTime() ) then
        return true
    end

    return ( ix.cityCode.GetCurrent() == "turmoil" or ix.cityCode.GetCurrent() == "jw" or ix.cityCode.GetCurrent() == "aj" or ix.cityCode.GetCurrent() == "void" )
end
...
function PLUGIN:Think()
    ...
    if inCombat then
        ...
        if not currentCombat or not currentCombat:IsPlaying() then
            ...
            currentCombat:ChangeVolume(ix.option.Get("musicCombatVol") / 200, 6)
    ...
    if not currentPassive or not currentPassive:IsPlaying() then
        ...
        currentPassive:ChangeVolume(ix.option.Get("musicAmbientVol") / 200, 12)
