...
if CLIENT then
    function ITEM:PaintOver(item, w, h)
        draw.SimpleText(
            item:GetData('stacks', 1).." / "..item.maxStacks, 'ixTinyFont', w - 5, h - 5,
...
ITEM.functions.split = {
    ...
    OnRun = function(item)
        ...
        if not ( item.invID and ix.inventory.Get(item.invID).GetReceivers and ix.inventory.Get(item.invID):GetReceivers() ) then return false end
        client:RequestString('Split', 'Please enter how many items you want to split', function(splitStack)
            ...
            if not ( ix.inventory.Get(item.invID).GetReceivers and ix.inventory.Get(item.invID):GetReceivers() ) then return false end
