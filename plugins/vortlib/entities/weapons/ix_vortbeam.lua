...
function SWEP:Think()
	if not ( self:GetOwner():IsVortigaunt() ) then
		self:Remove()
	end
end
...
function SWEP:Shoot(dmg, effect)
	...
	--local traceres = util.TraceHull(self.Owner:EyePos(), self.Owner:GetAimVector() * self.Range, self.Owner)
	self:ShootEffect(effect or "vortigaunt_beam",ply:EyePos(), traceres.HitPos)

	if ( SERVER ) then
		for k, v in pairs(ents.FindInSphere(traceres.HitPos, 96)) do
			if ( IsValid(v) and not ( v == ply ) ) then
