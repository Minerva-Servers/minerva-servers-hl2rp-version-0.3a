...
ix.config.Add("Shot Distance", 10, "How close a shot has to be to a part of the door to destroy that part", nil, {
    data = { min = 1, max = 10 },
    category = PLUGIN.name
})

ix.config.Add("Range", 300, "How close a person has to be to the door to shoot it open", nil, {
    data = { min = 1, max = 300 },
    category = PLUGIN.name
})

ix.config.Add("Respawn Timer", 180, "How long it takes for the door to respawn", nil, {
    data = { min = 1, max = 600 },
    category = PLUGIN.name
})
...
PLUGIN.blastableweapons = {
    ["weapon_shotgun"] = true,
    ["ix_spas12"] = true,
    ["arccw_minerva_spas12"] = true,
    ["arccw_ur_spas12"] = true,
    ["arccw_ur_db"] = true,
    ["arccw_ud_870"] = true,
    ["arccw_ud_m1014"] = true,
}
        ...
        if dmginfo:GetAttacker():IsPlayer() and dmginfo:GetInflictor():IsPlayer() then
            if self.ignoredamage[dmginfo:GetAttacker():GetActiveWeapon()] == nil then
                if target:GetClass() == "prop_door_rotating" and (target.canbeshot == nil or target.canbeshot == true) and IsValid(dmginfo:GetInflictor()) and dmginfo:IsBulletDamage() then
                    if dmginfo:GetInflictor():GetPos():Distance( target:GetPos() ) <= ix.config.Get("Range", 300) then
                        ...
                        if self.blastableweapons[dmginfo:GetAttacker():GetActiveWeapon():GetClass()] then
                            ...
                            Door:SetPos(target:GetPos() + target:GetUp() + dmginfo:GetAttacker():GetForward() * 32)
                            ...
                            Door:GetPhysicsObject():ApplyForceCenter( dmginfo:GetAttacker():GetForward() * 20000 )
                        ...
                        -- shooting lock
                        elseif distance <= ix.config.Get("Shot Distance", 10) then
                        ...
                        -- shooting hindges
                        elseif (dampos:Distance(hindge1) <= ix.config.Get("Shot Distance", 10)*1.5) then
                            ...
                            util.Effect("GlassImpact", effect, true, true)
                            target:EmitSound( "physics/wood/wood_crate_break"..math.random(1, 4)..".wav" , 80, 50, 1)
                        elseif (dampos:Distance(hindge2) <= ix.config.Get("Shot Distance", 10)*1.5) then
                        ...
                        if (target.bHindge1 and target.bHindge2) then
                            ...
                            Door:SetRenderMode(RENDERMODE_TRANSALPHA)
                            Door:Fire("unlock")
                            Door:Fire("openawayfrom", dmginfo:GetInflictor():UniqueID()..CurTime())
                            Door:Spawn()
                            target.canbeshot = false
                            target:SetPos(target:GetPos() + Vector(0,0,-1000))
                            timer.Simple(ix.config.Get("Respawn Timer", 180), function()
