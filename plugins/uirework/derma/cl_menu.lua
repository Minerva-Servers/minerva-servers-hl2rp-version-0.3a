...
function PANEL:Init()
	...
	self.bottomButtons:SetSize(self:GetWide(), 50)
	self.bottomButtons:Dock(BOTTOM)
	self.bottomButtons:SetPaintedManually(true)
	...
	local mainmenu = self.bottomButtons:Add("ixMenuButton")
	mainmenu:SetText("main menu")
	mainmenu:SetContentAlignment(6)
...
function PANEL:GetOverviewInfo(origin, angles, fov)
    local originAngles = Angle(0, angles.yaw - 10, angles.roll)
    ...
    local forward = originAngles:Forward() * 50 - originAngles:Right() * 20 + originAngles:Up() * 3
    ...
    if ( IsValid(target) ) then
        ...
        newOrigin = origin - LocalPlayer():OBBCenter() * 0.2 + forward
    end
    ...
    return LerpVector(fraction, origin, newOrigin), LerpAngle(fraction, angles, newAngles), Lerp(fraction, fov, 70), bDrawPlayer
end
...
function PANEL:GetStandardSubpanelSize()
	return ScrW() - self:GetPadding() * 2, ScrH() - self:GetPadding() * 6
end
...
function PANEL:PopulateTabs()
    ...
    self.bottomButtons:MoveToFront()
end
...
function PANEL:Paint(width, height)
	...
	self.bottomButtons:PaintManual()
