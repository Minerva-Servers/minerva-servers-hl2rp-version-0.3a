...
local backgroundColor = ix.config.Get("color", Color(115, 53, 142))
local dimColor = ix.config.Get("colorDim", Color(165, 134, 179))
...
function PANEL:OnKeyCodePressed(key)
    ...
    if ( key == KEY_SPACE and self.continueText > 0.25 ) then
        if not ( ix.data.Get("introPlayed") ) then
            ix.data.Set("introPlayed", true)
            
            if ( SCENES_PLAYING ) then
                return
            end
            
            ix.scenes.PlaySet(ix.data.Get("introSequence"), ix.data.Get("introMusic", ix.config.Get("music")), function()
                ix.gui.characterMenu = vgui.Create("ixCharMenu")
            end)
        else
            ix.gui.characterMenu = vgui.Create("ixCharMenu")
            ix.gui.characterMenu:PlayMusic()
            ix.gui.characterMenu:MoveToBack()
