...
function PANEL:Init()
	...
	self.leftPanel:SetWide(self:GetWide() / 2)

	self.rightPanel = self:Add("Panel")
	self.rightPanel:Dock(FILL)

    self.storageInventory = self.leftPanel:Add("ixInventory")
    ...
    self.storageInventory:SetIconSize(64)
    ...
    ix.gui.inv1 = self.rightPanel:Add("ixInventory")
...
function PANEL:SetLocalInventory(inventory)
    if (IsValid(ix.gui.inv1) and !IsValid(ix.gui.menu)) then
		...
		ix.gui.inv1:SetPos(self:GetWide() - ix.gui.inv1:GetWide(), 0)
    end
end
