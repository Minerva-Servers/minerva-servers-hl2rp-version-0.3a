...
function PANEL:Init()
    self:SetSize(ScreenScale(10), ScreenScale(10))
...
function PANEL:Init()
    ...
    self.icon = self:Add("AvatarImage")
    self.icon:Dock(LEFT)
    ...
    self.steamName = self:Add("DLabel")
    self.steamName:DockMargin(4, 0, 0, 0)
    self.steamName:Dock(LEFT)
...
function PANEL:Update()
    ...
    */
    ...
    for k, v in pairs(ix.plugin.list["badgeslib"].list) do
        if not ( v[3](client) ) then
            continue
        end
        
        local badges = self:Add("DImageButton")
        badges:SetPos(1830 - badgex, 5)
        badges:SetSize(24, 24)
        badges:SetImage(v[2])
        badges.DoClick = function()
            Derma_Message(v[1](client), "Minerva Servers", "Close")
        end
...
hook.Add("CreateMenuButtons", "ixScoreboard", function(tabs)
    tabs["scoreboard"] = {
        Create = function(info, container)
            container.ixscoreboard = container:Add("ixScoreboard")

            for i = 1, #container.ixscoreboard.factions do
                local factionPanel = container.ixscoreboard.factions[i]
