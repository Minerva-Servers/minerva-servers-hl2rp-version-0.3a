...
function ScannerLeave(ply)
    if (IsValid(ply.ixScn)) then
        ply.ixScn.spawn = ply.ixScnOldPos
        ply.ixScn:Remove()
        ply.ixScn = nil
...
function PLUGIN:PlayerSwitchFlashlight(ply, enabled)
    ...
    scanner:EmitSound("npc/turret_floor/click1.wav", 70, pitch)
    return false
end
