...
function ENT:ejectPilot()
    ...
    pilot:UnSpectate()
    local oldweps = pilot.ixScnOldWeapons

    if (not self.noRespawn and pilot:Alive()) then
        pilot:Spawn()

        if ( oldweps ) then
            for k, v in pairs(oldweps) do
                pilot:Give(v)
            end
        end

        if ( pilot.ixScnOldAmmo ) then
            for k, v in pairs(pilot.ixScnOldAmmo) do
                if not ( v and isnumber(v) ) then
                    continue
                end
        ...
        if ( pilot:GetWeapons() ) then
            for k, v in pairs(pilot:GetWeapons()) do
...
function ENT:flash()
    ...
    self:EmitSound("npc/scanner/scanner_photo1.wav", 70)
