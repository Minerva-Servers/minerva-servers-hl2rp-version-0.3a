...
function PANEL:SetSequence(key, data)
	...
	local label = vgui.Create("DLabel", self.main)
	label:SetPos(310, 5)
	label:SetText("Sequence: "..key)
	label:SetFont("ixMenuMiniFont")
	label:SizeToContents()

	local newSeq = vgui.Create("ixMenuButton", self.main)
	newSeq:SetPos(0, 0)
	newSeq:SetSize(75, 25)
	newSeq:SetText("Add")
	newSeq:SetFont("ixMenuMiniFont")
	newSeq:SetContentAlignment(5)
	...
	local remSeq = vgui.Create("ixMenuButton", self.main)
	remSeq:SetPos(75, 0)
	remSeq:SetSize(75, 25)
	remSeq:SetText("Remove")
	remSeq:SetFont("ixMenuMiniFont")
	remSeq:SetContentAlignment(5)
	...
	local saveSeq = vgui.Create("ixMenuButton", self.main)
	saveSeq:SetPos(150, 0)
	saveSeq:SetSize(75, 25)
	saveSeq:SetText("Save")
	saveSeq:SetFont("ixMenuMiniFont")
	saveSeq:SetContentAlignment(5)

	function saveSeq:DoClick()
		if not ix.ops.eventManager.sequences[key].FileName then
			Derma_StringRequest("Helix", "Enter sequence file name:", nil, function(name)
	...
	local uploadSeq = vgui.Create("ixMenuButton", self.main)
	uploadSeq:SetPos(225, 0)
	uploadSeq:SetSize(75, 25)
	uploadSeq:SetText("Push")
	uploadSeq:SetFont("ixMenuMiniFont")
	uploadSeq:SetContentAlignment(5)
...
function PANEL:AddEvent(id, eventdata)
	...
	function event.eprop:DoClick()
		...
		panel.Dad.Properties:SetSize(340, 580)
		...
		local noCol = l:Add("ixMenuButton")
		noCol:SetText("None")
		noCol:SetSize(40, 20)
	...
	local function moveToCustomSlot()
		Derma_StringRequest("Helix", "Enter where you wish to move this event to:", "", function(slot)
