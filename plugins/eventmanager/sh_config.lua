...
ix.ops.eventManager.config.Events = {
    ...
    ["explode_cinematic"] = {
        ...
        Clientside = false,
        Do = function(prop, uid)
            local tempEnt = ents.Create("ix_usable")
            ...
            timer.Simple(0.05, function()
                if IsValid(tempEnt) then
                    ix.ops.eventManager.MakeBigExplosion(tempEnt, prop["magnitude"], prop["debris"])
                end
            end)
        end
    },
    ...
    ["cineintro"] = {
        ...
        Clientside = false,
        Do = function(prop, uid)
            ix.ops.eventManager.cinematicIntro(prop["title"])
        end
    },
    ["screenfade"] = {
        Cat = "ui",
    ...
    ["soundplay"] = {
        ...
        Clientside = true,
        Do = function(prop, uid)
            if prop["combineonly"] and not LocalPlayer():IsCombine() then
                return
            end
    ...
    ["itemspawn"] = {
        ...
        Clientside = false,
        Do = function(prop, uid)
            local x = ix.item.Spawn(prop["itemclass"], prop["pos"])
            x:SetData("Restricted", true)
        end
    },
    ["ammobox_spawn"] = {
        Cat = "loot",
        ...
        Do = function(prop, uid)
            ...
            local ent = ents.Create("ix_ammobox")
            ent:SetPos(prop["pos"])
            ent:SetAngles(Angle(prop["ang"].x, prop["ang"].y, prop["ang"].z))
            ent:Spawn()
            ent:Activate()
    ...
    ["citycodeset"] = {
        ...
        Clientside = false,
        Do = function(prop, uid)
            ix.cityCode.Set(nil, prop[code] or "civil")
        end
    },
    ["voiceallowedset"] = {
        Cat = "server",
    ...
    ["combinewaypoint"] = {
        ...
        Do = function(prop, uid)
            if LocalPlayer():IsCombine() then
                Schema:AddWaypoint(prop["pos"], prop["message"], prop["col"], prop["duration"])
            end
        end
    },
}
